...
// Lab 5: Your code here:
    struct OpenFile* of;
    int check;
    if((check = openfile_lookup(envid, req->req_fileid, &of)) < 0)
        return check;
    off_t* offset = &of->o_fd->fd_offset;
    if((check = file_read(of->o_file, ret->ret_buf, MIN(req->req_n, PGSIZE),*offset)) < 0)
        return check;
    *offset += check;
	return check;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    if((check = openfile_lookup(envid, req->req_fileid, &of)) < 0)
        return check;
    off_t* offset = &of->o_fd->fd_offset;
    if((check = file_write(of->o_file, req->req_buf, req->req_n, *offset)) < 0)
        return check;
    offset += check;
    return check;
}
