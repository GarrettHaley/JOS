...
trap_dispatch(struct Trapframe *tf)
{
    ...
    if (tf->tf_trapno == T_SYSCALL) {
        ...
        print_trapframe(tf);
        if (tf->tf_cs == GD_KT)
            panic("ERROR: COULD NOT HANDLE EXCEPTION (kernel fault: trap.c)");
        else {
            env_destroy(curenv);
            return;
        }
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if (tf->tf_esp <= UXSTACKTOP && tf->tf_esp >= UXSTACKTOP - PGSIZE) {
        if (tf->tf_esp - UTrapsize - 4 >= UXSTACKTOP - PGSIZE)
            u_TF = (struct UTrapframe *)(tf->tf_esp - 4 - UTrapsize);
        else{
            cprintf("[%08x] user fault va %08x ip %08x\n",
            curenv->env_id, fault_va, tf->tf_eip);
            print_trapframe(tf);
        ...
        u_TF = (struct UTrapframe *)(UXSTACKTOP - UTrapsize);
